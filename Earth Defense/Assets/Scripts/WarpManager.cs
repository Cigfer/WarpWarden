...
public class WarpManager : MonoBehaviour {
	...
	void Update ()
    {
        if (Input.GetKeyDown("[1]"))
        {
            ...
            player.transform.rotation = tpLocations[0].transform.rotation;
        }
        if (Input.GetKeyDown("[2]"))
        {
            player.transform.position = tpLocations[1].transform.position;
            player.transform.rotation = tpLocations[1].transform.rotation;
        }
        if (Input.GetKeyDown("[3]"))
        {
            player.transform.position = tpLocations[2].transform.position;
            player.transform.rotation = tpLocations[2].transform.rotation;
        }
        if (Input.GetKeyDown("[4]"))
        {
            player.transform.position = tpLocations[3].transform.position;
            player.transform.rotation = tpLocations[3].transform.rotation;
        }
